...
2. **`scripts/subscription_unit_economics.py`** *(shipped 2026-06-29 per this tick — canonical 5th-layer Archetype A/B hybrid Path A/B/C subscription-program scorer)* — 12 input parameters (us_gmv + aov + monthly_orders + consumables_revenue_share_pct + aov_under_30 + has_replenishment_cadence + subscriber_conversion_baseline_pct + monthly_churn_baseline_pct + category + platform_preference + has_subscriber_attribution + operator_capacity_hours_per_week) → outputs Path A (Recharge Starter $99/mo $100k-$500k) / Path B (Recharge Plus DEFAULT $499/mo $500k-$10M) / Path C (Recharge Enterprise OR Skio Enterprise OR Seal Subscriptions OR Stay AI $1k-$5k/mo $10M+) recommendation with cost stack + Year-1 subscription revenue + 2.0-3.0× LTV multiplier Path B + smart-cancellation recovery 20-35% + dunning recovery 50-70% + winback recovery 10-20% + 5-discount-tier matrix [30-day 5% / 45-day 10% / 60-day 15% DEFAULT / 90-day 20% / 120-day 25%] + 6-step build sequence. 94 TDD tests across 11 test classes; hermetic local-config scorer with no Recharge / Skio / Klaviyo / Triple Whale API access required. Gated on asset 16 + /subscriptions route both shipping first — both shipped 2026-06-29 per this tick.
3. **`dashboards/subscription-program-health.html`** *(shipped 2026-06-29 per this tick — canonical 6th-and-final static-dashboard layer)* — render subscription-launch readiness + subscriber-cohort LTV + churn-by-tier + dunning-recovery-rate + replenishment-conversion-rate as a 1-click operator surface. Self-contained static HTML ~40KB / 6 sections + 4 canonical data structures [FLOWS 5 flows + PATH_TABLE 3 path tiers + PHASE_GATES 4 phases + SAMPLE_INPUTS Path B default $3M US base] + 100 Node smoke tests across 26 categories. Mirrors the v0.11.0 static-dashboard-tick recipe. The subscription-program track is now 6/6 layers complete and fully closed per the v0.11.0 track-fully-closed pivot pattern.
...
**Total voice-driven cells: 50** (5 flows × 5 voices × {email + SMS}). **Canonical subscription-program track layer count after this asset: 4/6** (research/08 + playbook/15 + asset 16 + /subscriptions route + scripts/subscription_unit_economics.py all shipped 2026-06-29; 1 remaining layer per canonical layer order: subscription-program-health.html).