...
1. **`dashboard/app/subscriptions/page.tsx`** *(canonical 4th-layer Next.js operator-surface route)* — render research/08 + playbook/15 + asset 16 as a unified subscription-launch readiness heat-map (canonical from the `research → playbook → asset → operator-surface → scripts → static-dashboard` layer order). ~1.5 hr bounded. Gated on asset 16 shipping first.
