...
3. **`dashboards/attribution-health-alert-archive.html`** (canonical 6th-and-final static-dashboard layer — 13th static dashboard in the workspace; self-contained static HTML ~41KB / 6 sections + 4 canonical data structures [PATH_TABLE 5 path tiers A/B/C/D/E with paid-spend floor + cadence + cooldown + platforms + ROI band / WEBHOOK_THRESHOLDS 5 canonical webhook thresholds pinned by _canonical_thresholds_published() regression gate per scripts/attribution_health_alert_webhook.py / ALERT_PAYLOAD_FIELDS 13 canonical alert-payload fields pinned by _canonical_alert_shape_published() regression gate / DECISION_RULES 5-rule engine (R1 ANY per-platform fail / R2 cross-platform drift / R3 match-rate drift > 3.0pp / R4 coverage drift > 2.0pp / R5 cooldown suppression 3600s) + 7 archive entries (5 canonical root-cause hypotheses from scripts/attribution_cross_platform_rollup.py lines 61-89 + 2 baseline-fire patterns)] + ~135-140 Node smoke tests across 28 categories; compounds the 5 prior Move #6.10 artifacts as a 1-click attribution-alert-archive browser for $1M-$5M Path B brands; **SHIPPED 2026-07-10 per the static-dashboard-tick follow-up to this tick + Move #6.10.4 tick** — the Move #6.10 attribution-health alert webhook track is now **6/6 layers complete — TRACK FULLY CLOSED 2026-07-10** per the v0.11.0 track-fully-closed pivot pattern per the canonical ship-everything-this-tick pattern).
