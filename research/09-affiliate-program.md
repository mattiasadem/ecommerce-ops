...
1. **`playbooks/16-affiliate-program-launch.md`** (canonical 2nd-layer operator-build companion — the 4-phase Recruit → Onboard → Cookie-mitigate → Tier-promote recipe with step-by-step Refersion + Levanta + Triple Whale + Klaviyo + PayPal wiring across 30 days; gated on this research/09 being live).
2. **`assets/17-affiliate-program-templates.md`** (canonical 3rd-layer operator-copy companion — the paste-ready affiliate-application landing-page template + 5 voice-variant welcome-email templates + 5 voice-variant recruitment-email templates + FTC disclosure language templates + 3-clause affiliate agreement template + 4-trigger tier-promotion SOP template; compounds `assets/10-affiliate-program-playbook.md` 5-voice commission-tier matrix + `assets/03-ugc-brief.md` creator outreach emails + contract templates + `assets/12-impact-data-pipeline.md` Sustainable-mission-verifier).
3. **`dashboard/app/affiliates/page.tsx`** (canonical 4th-layer Next.js operator-surface route — renders research/09 + playbook 16 + asset 17 as a unified affiliate-launch readiness heat-map with 4 hero metrics + TL;DR + 3 layer cards + 5-voice density pills + future-tick companions; gated on playbook 16 + asset 17 both being live).
4. **`scripts/affiliate_unit_economics.py`** (canonical 5th-layer Archetype A/B hybrid Path A/B/C scorer — takes operator-supplied gmv + aov + expected_affiliate_count + commission_tier + voice_profile + has_triple_whale + has_klaviyo_post_purchase + iq_zone + operator_capacity_hours_per_week → outputs Path A (GoAffPro free) / Path B (Refersion DEFAULT) / Path C (Impact + Levanta) recommendation with cost stack + Year-1 incremental revenue + LTV-multiplier + cookie-loss-recovery-rate + 6-step build sequence; gated on dashboard/app/affiliates/page.tsx being live).
