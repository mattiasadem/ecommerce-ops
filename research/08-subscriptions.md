...
>
> **Companion artifacts in this workspace.** This is the research synthesis layer — the canonical 1st-layer follow-up per the canonical research → playbook → asset → operator-surface → scripts → static-dashboard layer order. The subscription-program track is now fully closed across all 6 canonical layers (research/08 + playbook 15 + asset 16 + /subscriptions route + scripts/subscription_unit_economics.py + dashboards/subscription-program-health.html all shipped 2026-06-29): *playbooks/15-subscription-program-launch.md* — shipped 2026-06-29 per the playbook-tick follow-up to this research doc — the canonical 2nd-layer operator-build companion per the canonical research → playbook → asset → operator-surface → scripts → static-dashboard layer order; ~58KB / 490 lines / 4-phase Recharge + Skio + Bold + Stay AI + Appstle + Seal + Loop operator build across 3 GMV-tier paths with 7-platform decision matrix + 5-discount-tier matrix + 4-alternative smart-cancellation flow + 3-attempt dunning-flow + 60-day winback-flow + 10-metric operational KPI dashboard + 15 pitfalls with Fix lines + 8.3:1 Year-1 ROI Path B): *assets/16-subscription-flow-templates.md* (paste-ready per-flow email + SMS subscription templates × 5 voice profiles) — shipped 2026-06-29 per this tick per the canonical 3rd-layer operator-copy companion to research/08 + playbook 15 + *dashboard/app/subscriptions/page.tsx* (Next.js operator-surface route rendering research/08 + playbook 15 + asset 16 as a unified subscription-launch readiness heat-map) — shipped 2026-06-29 per this tick per the canonical 4th-layer operator-surface companion to research/08 + playbook 15 + asset 16 + *scripts/subscription_unit_economics.py* (Archetype A/B hybrid Path A/B/C subscription-program scorer: takes consumables-revenue-share + sku-purchase-cadence + LTV baseline + churn-baseline + subscriber-conversion-rate → outputs Path A (Recharge Starter) / Path B (Recharge Plus DEFAULT) / Path C (Skio + Stay AI + custom) recommendation with cost stack + expected Year-1 incremental subscription-revenue + LTV multiplier) — shipped 2026-06-29 per the script-tick follow-up to this research doc + playbook 15 + asset 16 + /subscriptions route — canonical 5th-layer Archetype A/B hybrid Path A/B/C scorer (16th script in the workspace; 94 TDD tests across 11 test classes; hermetic local-config scorer with no Recharge / Skio / Klaviyo / Triple Whale API access required; 12 input parameters → Path A Recharge Starter / Path B Recharge Plus DEFAULT / Path C Recharge Enterprise OR Skio Enterprise OR Seal Subscriptions OR Stay AI recommendation with cost stack + Year-1 subscription revenue + 2.0-3.0× LTV multiplier Path B + smart-cancellation recovery 20-35% + dunning recovery 50-70% + winback recovery 10-20% + 5-discount-tier matrix [30-day 5% / 45-day 10% / 60-day 15% DEFAULT / 90-day 20% / 120-day 25%] + 6-step build sequence; compounds this research doc + playbook 15 + asset 16 by automating the per-brand path-selection decision the operator currently does manually against the 3-path GMV-tier matrix) + *dashboards/subscription-program-health.html* (canonical 6th-and-final static-dashboard layer; renders subscription-launch readiness + subscriber-cohort LTV + churn-by-tier + dunning-recovery-rate + replenishment-conversion-rate as a 1-click operator surface) — shipped 2026-06-29 per the dashboard-tick follow-up to this research doc + playbook 15 + asset 16 + /subscriptions route + the script — self-contained static HTML ~40KB / 6 sections + 4 canonical data structures [FLOWS 5 flows subscription-welcome + replenishment-reminder + pause-reactivation + cancellation-confirmation + winback with per-flow pathA/pathB/pathC tier + smart-cancel/dunning/winback contribution percentages + PATH_TABLE 3 path tiers with cost stack + LTV multiplier + subscriber conversion + smart-cancel/dunning/winback recovery + 5-discount-tier matrix + PHASE_GATES 4 phases with 10/10/10/9 prereqs + SAMPLE_INPUTS Path B default $3M US base / 70% consumables-revenue-share / $75 AOV / 3,000 monthly orders / 18% subscriber-conversion-baseline / 7% monthly-churn-baseline / 4 hr/wk operator capacity] + 8 helper functions (pathForGmv + year1RevShareLow/High + year1CostLow/High + fmtUSD + fmtPct + statusClass + statusLabel) + 6 render functions (renderSummary + renderFlowGrid + renderPathChart + renderPhaseGates + renderFlowTable + renderNextAction) + AbortController 1500ms fetch-timeout contract + URL param parsing [?path= ?us_gmv= ?demo=1] + 100 Node smoke tests across 26 categories; renders subscription-launch readiness (5 flows × {live/draft/staging} status) + per-path Year-1 incremental subscription revenue (per-path market-share weights) + 4-phase gate status (Phase 1 PDP widget 10 prereqs + Phase 2 customer-portal 10 prereqs + Phase 3 replenishment 10 prereqs + Phase 4 subscriber-cohort analytics 9 prereqs per playbook 15 §Verification gates 10/10/10/9 prereqs) + per-flow readiness detail table (5 flows × 6 columns) + recommended next-action CTA as the 1-click operator surface for tracking Phase 1 PDP widget + Phase 2 customer-portal + Phase 3 replenishment + Phase 4 subscriber-cohort analytics gate progress against the 3-path GMV-tier decision matrix).
