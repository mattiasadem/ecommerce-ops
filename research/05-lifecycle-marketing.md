...
>
> **Companion artifacts in this workspace.** This is the **synthesis layer** for the lifecycle-marketing expansion track. The canonical next-layer follow-ups (per the v0.9.0 layer-order-completion sub-rule research → playbook → asset → operator-surface → scripts) are: `playbooks/12-lifecycle-flow-library.md` *(planned — does not yet exist)* — paste-ready Klaviyo + Postscript + Smile wiring for all 20 flows + segment definitions + cadence rules + verification gates; `assets/14-lifecycle-flow-templates.md` *(planned — does not yet exist)* — the per-flow paste-ready email + SMS templates × 5 voice-driven override columns = 20 × 5 = 100 voice-variant templates; `dashboard/app/lifecycle/page.tsx` *(planned — does not yet exist)* — the Next.js operator-surface route surfacing the library as a 1-click per-flow wiring diagram.
...
1. **Companion playbook — `playbooks/12-lifecycle-flow-library.md`** *(planned — does not yet exist)* — paste-ready Klaviyo + Postscript + Smile wiring for all 20 flows; ~1.5 hr bounded; mirrors the 16-shipped-playbook pattern with 6-step build per flow + 7-gate verification + 15-pitfall list + ROI table. Compounds research/05 by mapping each pillar + each flow into a step-by-step operator build.
2. **Companion asset — `assets/14-lifecycle-flow-templates.md`** *(planned — does not yet exist)* — the per-flow paste-ready email + SMS templates × 5 voice-driven override columns = 20 × 5 = 100 voice-variant paste-ready templates; ~1.5 hr bounded; mirrors the 13-shipped-asset pattern with 7 dimensions × 5 voices + paste-ready subject-line + pre-header + body templates. Compounds research/05 + playbook 12 by giving the operator the actual copy to paste into each flow.
3. **Companion script — `scripts/lifecycle_flow_health_check.py`** *(planned — does not yet exist)* — Archetype C/D-light hybrid check that audits each of the 13 live flows against the canonical KPI benchmarks (per-flow revenue per 1k events + open-rate + CTR + unsubscribe-rate + flow-attribution match rate); flags any flow outside ±20% of the benchmark for the operator's review. Mirrors the Move #6.5 attribution-quality-audit pattern with 6 gates × 13 flows = 78 gate-flow combinations.
...
- `playbooks/12-lifecycle-flow-library.md` *(planned — does not yet exist)* — paste-ready Klaviyo + Postscript + Smile wiring for all 20 flows + segment definitions + cadence rules + verification gates
- `assets/14-lifecycle-flow-templates.md` *(planned — does not yet exist)* — the per-flow paste-ready email + SMS templates × 5 voice-driven override columns = 20 × 5 = 100 voice-variant templates
- `scripts/lifecycle_flow_health_check.py` + `scripts/tests/test_lifecycle_flow_health_check.py` *(planned — does not yet exist)* — Archetype C/D-light hybrid check that audits each of the 13 live flows against the canonical KPI benchmarks (6 gates × 13 flows = 78 gate-flow combinations)
