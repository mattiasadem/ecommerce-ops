...
2. **Companion asset `assets/15-marketplace-listing-card.md`** *(planned — does not yet exist)* — paste-ready per-marketplace per-category listing card with title-template + bullet-point-template + A+ content skeleton + per-voice per-marketplace description variant + keyword-stuffing guardrail (~2 hr).
...
5. **Companion operator-surface route `dashboard/app/marketplace/page.tsx`** *(planned — does not yet exist)* — Next.js dashboard route surfacing the framework as a 1-click per-marketplace launch readiness heat-map + per-marketplace revenue contribution + 4-phase gate status + DTC-cannibalization overlay (~45 min).
...
- `dashboard/app/lifecycle/page.tsx` — Next.js operator-surface route for the lifecycle-marketing track (companion surface for the planned `dashboard/app/marketplace/page.tsx` route)
