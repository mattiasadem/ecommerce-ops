...
>
> **Companion artifacts:** `assets/14-lifecycle-flow-templates.md` *(planned — does not yet exist)* — the per-flow email + SMS templates × 5 voice-driven override columns = 100 voice-variant paste-ready templates (the operator-copy layer that compounds this playbook). `scripts/lifecycle_flow_health_check.py` *(planned — does not yet exist)* — Archetype C/D-light hybrid check that audits each of the 20 flows against the canonical KPI benchmarks (78 gate-flow combinations).
>
...
`scripts/lifecycle_flow_health_check.py` *(planned — does not yet exist)* — Archetype C/D-light hybrid audit script that checks each of the 20 flows against the canonical KPI benchmarks from the Metrics table (open rate / click rate / CVR / unsubscribe rate / cohort LTV). The script would score each flow on a 0–100 scale and emit a prioritized fix-list. Pattern mirrors `scripts/triple_whale_attribution_check.py` and `scripts/checkout_audit_score.py` (gated on the 20 flows being live for ≥30 days before the script can score them — the script is the 90-day-post-launch audit tool, not the launch-time tool).

---

## Next moves after this lifecycle library is live

1. **Companion asset `assets/14-lifecycle-flow-templates.md`** *(planned — does not yet exist)* — the per-flow email + SMS templates × 5 voice-driven override columns = 20 × 5 = 100 voice-variant paste-ready templates. Compounds this playbook by shipping the operator-copy layer.
2. **Companion script `scripts/lifecycle_flow_health_check.py`** *(planned — does not yet exist)* — the 78 gate-flow audit (20 flows × 6 gates each = 120 gate-flow combinations; the canonical Archetype C/D-light hybrid pattern from Move #6.5 + #6.6 + #6.7 + #6.8).
