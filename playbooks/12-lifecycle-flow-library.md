...
3. **Companion static dashboard `dashboards/lifecycle-flow-library.html`** *(shipped 2026-06-27 per the dashboard-tick follow-up to research/05 + playbook/12 + asset/14 + scripts/lifecycle_flow_health_check.py — the canonical 6th-and-final layer of the lifecycle-marketing track per the v0.11.0 extended layer order research → playbook → asset → operator-surface-route → script → static-dashboard; 6 sections × 4 canonical data structures × 8 helper functions × 6 render functions; 100-test Node smoke suite; compounds this playbook + the script by visualizing all 5 prior artifacts as a 1-click launch-readiness heat-map against the canonical 78 gate-flow combinations)* — the static HTML dashboard rendering the 20-flow library + per-flow KPI scorecard + per-tier revenue contribution + 4-tier launch-ladder progress.
