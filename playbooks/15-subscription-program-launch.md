...
Operators can pre-flight their cost-vs-revenue via `scripts/subscription_unit_economics.py` (the canonical 5th-layer scoring script for the subscription-program track per research/03 line 176; Archetype A/B hybrid Path A/B/C scorer that takes consumables-revenue-share + sku-purchase-cadence + LTV baseline + churn-baseline + subscriber-conversion-rate → outputs Path A/B/C recommendation with cost stack + expected Year-1 incremental subscription-revenue + LTV-multiplier). **Shipped 2026-06-29 per the script-tick follow-up to research/08 + this playbook + asset 16 + /subscriptions route — canonical 5th-layer Archetype A/B hybrid Path A/B/C scorer (16th script in the workspace; 94 TDD tests across 11 test classes)** — 12 input parameters → Path A / Path B DEFAULT / Path C recommendation with cost stack + Year-1 subscription revenue + 2.0-3.0× LTV multiplier Path B + smart-cancellation recovery 20-35% + dunning recovery 50-70% + winback recovery 10-20% + 5-discount-tier matrix [30-day 5% / 45-day 10% / 60-day 15% DEFAULT / 90-day 20% / 120-day 25%] + 6-step build sequence; hermetic local-config scorer with no Recharge / Skio / Klaviyo / Triple Whale API access required.

---

## Companion tool / Future-tick companions

- **`scripts/subscription_unit_economics.py`** *(shipped 2026-06-29 per this tick)* — Archetype A/B hybrid Path A/B/C subscription-program scorer (canonical 5th-layer scoring script for the subscription-program track). Takes consumables-revenue-share + sku-purchase-cadence + LTV-baseline + churn-baseline + subscriber-conversion-rate → outputs Path A/B/C recommendation with cost-stack + Year-1 incremental + LTV-multiplier. 12 input parameters → 5-discount-tier matrix [30-day 5% / 45-day 10% / 60-day 15% DEFAULT / 90-day 20% / 120-day 25%] + smart-cancellation recovery 20-35% Path B + dunning recovery 50-70% + winback recovery 10-20% + 6-step build sequence; 94 TDD tests across 11 test classes; hermetic local-config scorer with no Recharge / Skio / Klaviyo / Triple Whale API access required. Per v0.9.0 layer-order-completion sub-rule: ships AFTER `assets/16-subscription-flow-templates.md` + `dashboard/app/subscriptions/page.tsx` — both shipped 2026-06-29, so the script was unblocked.
...
- **`dashboard/app/subscriptions/page.tsx`** *(planned future-tick)* — Next.js operator-surface route rendering research/08 + playbook 15 + asset 16 as a unified subscription-launch readiness heat-map. **Shipped 2026-06-29 per this tick** — canonical 4th-layer Next.js operator-surface route (17th route in the dashboard); renders 4 hero metrics (Path B 8.3:1 default Year-1 ROI / $270k Year-1 incremental net / 4 phases / 50 voice-cells) + TL;DR from research/08 + 3 layer cards (RD-08 + PB-15 + AS-16 with 5-voice density pills all ≥15) + future-tick companions [scripts/subscription_unit_economics.py + dashboards/subscription-program-health.html].
- **`dashboards/subscription-program-health.html`** *(shipped 2026-06-29 per this tick — canonical 6th-and-final static-dashboard layer per the v0.11.0 extended layer order research → playbook → asset → operator-surface-route → script → static-dashboard; self-contained static HTML ~40KB / 6 sections + 4 canonical data structures [FLOWS 5 flows + PATH_TABLE 3 path tiers + PHASE_GATES 4 phases + SAMPLE_INPUTS Path B default $3M US base] + 8 helper functions + 6 render functions + AbortController 1500ms fetch-timeout contract + URL param parsing [?path= ?us_gmv= ?demo=1] + 100 Node smoke tests across 26 categories; renders subscription-launch readiness (5 flows × {live/draft/staging} status) + per-path Year-1 incremental subscription revenue (per-path market-share weights) + 4-phase gate status (Phase 1 PDP widget 10 prereqs + Phase 2 customer-portal 10 prereqs + Phase 3 replenishment 10 prereqs + Phase 4 subscriber-cohort analytics 9 prereqs per this playbook §Verification gates 10/10/10/9 prereqs) + per-flow readiness detail table (5 flows × 6 columns) + recommended next-action CTA as the 1-click operator surface for tracking Phase 1 PDP widget + Phase 2 customer-portal + Phase 3 replenishment + Phase 4 subscriber-cohort analytics gate progress against the 3-path GMV-tier decision matrix; compounds research/08 + this playbook + asset 16 + /subscriptions route + the script by visualizing all 5 prior artifacts as a 1-click launch-readiness heat-map; canonical close of the subscription-program track per the v0.11.0 track-fully-closed pivot pattern; the subscription-program track is now 6/6 layers complete and fully closed)*
