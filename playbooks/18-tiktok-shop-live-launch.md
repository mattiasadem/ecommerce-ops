...
2. **`dashboard/app/tiktok/page.tsx`** *(planned — does not yet exist)* — the canonical 4th-layer Next.js operator-surface route. Renders research/11 + this playbook + assets/19 as unified operator surface with 4 hero metrics [Path B 8.5:1 default Year-1 ROI / 4.8+ Shop-Score target / 4-phase LIVE cadence / 30 voice-cells] + TL;DR from research/11 + 3 layer cards [RD-11 + PB-18 + AS-19 with 5-voice density pills all ≥15] + future-tick companions [scripts/tiktok_shop_unit_economics.py + dashboards/tiktok-shop-live-commerce-health.html]. **Gated on this playbook + assets/19 shipping first per canonical layer order.**
...
4. **`dashboards/tiktok-shop-live-commerce-health.html`** *(planned — does not yet exist)* + **`dashboards/tests/test_tiktok_shop_live_commerce_health.js`** *(planned — does not yet exist)* — the canonical 6th-and-final static-dashboard layer. Self-contained static HTML ~40KB / 6 sections + 4 canonical data structures [FLOWS 5 flows + PATH_TABLE 3 path tiers A/B/C + PHASE_GATES 4 phases with 10/10/10/9 prereqs each + SAMPLE_INPUTS Path B default $2M US DTC base / 35 SKUs / 50% gross margin / 4 hr/wk LIVE capacity / creator-affiliate-pool ≥20 / 4.5+ Shop-Score baseline] + 8 helper functions + 6 render functions + URL param parsing [?path= ?us_dtc_gmv= ?demo=1] + 5 canonical TikTok-Shop-flows × {live/draft/staging} readiness + 3-path GMV-tier decision matrix + 4 phase-by-phase verification gates with 10/10/10/9 prereqs each mirroring this playbook §Phase 1+2+3+4 verification gates + 60-80 Node smoke tests across 26 categories per the canonical v0.11.0 static-dashboard-tick recipe. **Gated on this playbook + assets/19 + dashboard/app/tiktok/page.tsx + scripts/tiktok_shop_unit_economics.py shipping first per canonical layer order.**
...
- **`dashboard/app/tiktok/page.tsx`** *(planned — does not yet exist)* — canonical 4th-layer Next.js operator-surface route. Renders research/11 + this playbook + assets/19 as unified operator surface.
...
- **`dashboards/tiktok-shop-live-commerce-health.html`** *(planned — does not yet exist)* + **`dashboards/tests/test_tiktok_shop_live_commerce_health.js`** *(planned — does not yet exist)* — canonical 6th-and-final static-dashboard layer.
